"""
tests/test_phase23_1_realism.py

Verification tests for Phase 23.1:
- Reconciles truth lock reproduction of Precision Fusion 1.2.
- Verifies manifest hashes match files on disk.
- Verifies presence and counts of all 11 proof files.
- Verifies behavioral diversity has more than 1 unique behavior.
...
class TestPhase231ProofFiles:
    ...
    def test_manifest_hashes_match(self):
        manifest = _load_json("phase23_1_audit_manifest.json")
        keys = [
            ("phase23_1_overlay_accounting_hash", "phase23_1_overlay_accounting.csv"),
            ("phase23_1_funding_extreme_skip_audit_hash", "phase23_1_funding_extreme_skip_audit.csv"),
            ("phase23_1_false_breakout_filter_audit_hash", "phase23_1_false_breakout_filter_audit.csv"),
            ("phase23_1_weak_continuation_audit_hash", "phase23_1_weak_continuation_audit.csv"),
            ("phase23_1_expansion_layer_reconciliation_hash", "phase23_1_expansion_layer_reconciliation.csv"),
            ("phase23_1_behavioral_dedup_root_cause_hash", "phase23_1_behavioral_dedup_root_cause.csv"),
            ("phase23_1_behavioral_diversity_test_hash", "phase23_1_behavioral_diversity_test.csv"),
            ("phase23_1_research_only_library_hash", "phase23_1_research_only_library.csv"),
            ("phase23_1_negative_zero_month_impact_hash", "phase23_1_negative_zero_month_impact.csv")
        ]
...
class TestPhase231BehavioralDiversity:
    def test_diversity_count(self):
        rows = _load_csv("phase23_1_behavioral_diversity_test.csv")
        assert len(rows) == 10, f"Must have exactly 10 overlays, got {len(rows)}"
        hashes = [r["unique_behavior_hash"] for r in rows]
        assert len(hashes) == len(set(hashes)), "Unique behavior hashes must be distinct"
        assert len(set(hashes)) > 1, "Behavioral diversity collapsed to a single behavior!"

class TestPhase231NoLookahead:
