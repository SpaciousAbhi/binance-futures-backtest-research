...
> The candidate expansion sweep evaluated **150 candidate combinations** across multiple parameter ranges (ADX, Expected R thresholds, session filters). However, because no tested routing candidate could improve monthly trade count layers (400+, 500+, 650+) without violating the benchmark quality gates (PF $\ge 2.20$ preferred / 2.00 minimum, Max DD $\le 12.0\%$), we honestly **retained Precision Fusion 1.2** as the final selected benchmark strategy to protect capital and code integrity.
